function [Gd_in, p_total, ray_directions, slowness_total, n_total, wave_number] = fn_2D_anisotropic_greens(a, C, rho, f, in_wave, d)
%SUMMARY
...
group_velocity_total(:,in_wave)'*sqrt(d)).*exp(1i*(wave_number(:,in_wave)'*d+0.5*pi*sig_0)); % -w.*t %
Gd_in = Gd_in';
...
ray_directions = atan2(d1,vgr_total(:,2,in_wave)).*sign(vgr_total(:,1,in_wave));
%ray_directions = flip(ray_directions);
